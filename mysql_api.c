...
void insert_config_main_power(char* server, char* user ,char* password ,char* dbname,char* nm_table, char* column[], const char* data[]
...
{      
  ...
  snprintf(temp, sizeof temp, ",id_location) values("); 
  strcat(select, temp);
  for(i=0;i<n_column;i++)
  {
		if(column_type[i] == 0){
